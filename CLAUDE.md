...
3. **Achievements** — `recent-passes.tsx`, a celebration wall: the twenty most recent validated projects as two rows of ten circular portraits with their mark. Exam ranks (`ProjectPass.isExam`) get a gold ring and a pure-CSS firework burst behind the portrait: two rings of bubbles (22 outer + 12 inner, mixed sizes, some hollow) whose keyframes live in `globals.css`, staggered per card, with the outer ring capped at a 96px radius so a burst never reaches the next face. Under `prefers-reduced-motion` the sparks are hidden and the gold ring plus a static glow still mark the exams, so the distinction is never animation-only.
